...
import { putUpdateClinic } from '../../../../services/clinicService';
...
const UpdateClinic = (props) => {
    ...
    const handleSubmitUpdate = async () => {
        if (!name) {
            ...
            toast.warn("Mô tả của phòng khám không hợp lệ")
            return;
        }
        else if (!address) {
            toast.warn("Dịa chỉ không hợp lệ")
            return;
        }



        let result = await putUpdateClinic(dataUpdate.id, name, description, address, image);
        if (result.ER === 0) {
            ...
            {dataUpdate ?
                ...
                >
                    <DialogTitle sx={{ m: 0, p: 2, fontSize: "20px", fontWeight: "500" }} id="customized-dialog-title">
                        Cập nhật thông tin phòng khám số {dataUpdate.id}
                    </DialogTitle>
